# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 14:23:06 2018
...
def linearRegression(x1,x2,x3,x4,y,x1_test,x2_test,x3_test,x4_test,y_test):
    ...
    #Run gradient descent with regularization on 500 iterations
    c0,c1,c2,c3,c4=gradient_descent_with_regularization(x0,x1,x2,x3,x4,y,c0,c1,c2,c3,c4,samples,alpha,500,l)
...
#function to calculate cost_function
...
def gradient_descent_with_regularization(x0,x1,x2,x3,x4,y,c0,c1,c2,c3,c4,samples,alpha,iterations,l): 
    for i in range(iterations):
        #Hypothesis
        h=(x0*c0)+(x1*c1)+(x2*c2)+(x3*c3)+(x4*c4)
        #Update the coefficients
        c0=c0*(1-(alpha*l)/samples)-(alpha/samples)*(sum(x0*(h-y)))
        c1=c1*(1-(alpha*l)/samples)-(alpha/samples)*(sum(x1*(h-y)))
        c2=c2*(1-(alpha*l)/samples)-(alpha/samples)*(sum(x2*(h-y)))
        c3=c3*(1-(alpha*l)/samples)-(alpha/samples)*(sum(x3*(h-y)))
        c4=c4*(1-(alpha*l)/samples)-(alpha/samples)*(sum(x4*(h-y)))
        #cost_function(x0,x1,x2,x3,x4,y,c0,c1,c2,c3,c4,samples)
