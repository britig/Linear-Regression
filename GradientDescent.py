...
#function to calculate gradient_descent_without_regularization
def gradient_descent_without_regularization(x0,x1,x2,x3,x4,y,
                                            c0,c1,c2,c3,c4,samples,alpha,iterations,
                                            x1_test,x2_test,x3_test,x4_test,y_test): 
    ...
    for i in range(iterations):
        ...
        c0=c0-(alpha/samples)*(sumM(x0*(h-y)))
        c1=c1-(alpha/samples)*(sumM(x1*(h-y)))
        c2=c2-(alpha/samples)*(sumM(x2*(h-y)))
        c3=c3-(alpha/samples)*(sumM(x3*(h-y)))
        c4=c4-(alpha/samples)*(sumM(x4*(h-y)))
        y_pred=x0[0:4322]*(c0[0:4322])+x1_test*(c1[0:4322])+x2_test*(c2[0:4322])+x3_test*(c3[0:4322])+x4_test*(c4[0:4322])
        rm = rmse(y_test,y_pred)
        cost_history[i] = rm
        
    return c0,c1,c2,c3,c4,cost_history
