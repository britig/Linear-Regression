# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 15:11:30 2018
...
def linearRegression(x1,x2,x3,x4,y,x1_test,x2_test,x3_test,x4_test,y_test):
    ...
    #Run gradient descent without regularization on 10 iterations
    c0,c1,c2,c3,c4=gradient_descent_without_regularization(x0,x1,x2,x3,x4,y,c0,c1,c2,c3,c4,samples,alpha,100)
    #print(c1)
...
def gradient_descent_without_regularization(x0,x1,x2,x3,x4,y,c0,c1,c2,c3,c4,samples,alpha,iterations): 
    for i in range(iterations):
        ...
        c0=c0-(alpha/samples)*(np.sum(x0*diff))
        c1=c1-(alpha/samples)*(np.sum(x1*diff))
        c2=c2-(alpha/samples)*(np.sum(x2*diff))
        c3=c3-(alpha/samples)*(np.sum(x3*diff))
        c4=c4-(alpha/samples)*(np.sum(x4*diff))
        #cost_function(x0,x1,x2,x3,x4,y,c0,c1,c2,c3,c4,samples)
